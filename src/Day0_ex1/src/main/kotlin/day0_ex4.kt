fun main(args: Array<String>) {
    ...
    println("Enter a humidity:")
    var humidity = readlnOrNull()?.toDoubleOrNull()
    while (humidity == null || humidity > 100 || humidity < 0) {
        println("Incorrect input. Enter a humidity:")
        humidity = readlnOrNull()!!?.toDouble()
    }
    ...
    comfortableHumidity (season, humidity)
...
private fun comfortableHumidity (season: Season, value: Double) {
    var lowerHumidity: Double = 0.0
    var upperHumidity: Double = 0.0
    when (season) {
        Season.WINTER -> {
            lowerHumidity = 30.0
            upperHumidity = 45.0
    ...
    println("The comfortable humidity is from $lowerHumidity% to $upperHumidity%")
    if (value < lowerHumidity || value > upperHumidity) {
